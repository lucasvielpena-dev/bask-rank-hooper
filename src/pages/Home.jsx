...
export default function Home({ profile, onNavigate }) {
  ...
  const statsData = useMemo(() => [
    { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, value: stats.jogadores, label: 'Atletas' },
    { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>, value: stats.mediaGeral.toFixed(1), label: 'Nota' },
    { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 1 0 20"/><path d="M2 12h20"/></svg>, value: stats.avaliacoes, label: 'Partidas' },
      ...
      <div className="home-container">
        ...
        <div className="home-position-card">
          ...
          {myPlayerInfo?.foto_perfil ? (
            ...
            <div className="home-position-avatar-placeholder">
          ...
          <div className="home-highlights-scroll">
            {topHighlights.map((item, i) => (
              <div key={i} className="home-highlight-card">
                <div className={`home-highlight-rank ${item.rankClass}`}>{item.rank}</div>
                <div className="home-highlight-name">{item.name}</div>
                <div className="home-highlight-detail">{item.pos}</div>
                <div className="home-highlight-score">{item.nota}</div>
              </div>
            ))}
          </div>
        </div>
